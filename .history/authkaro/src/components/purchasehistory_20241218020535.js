  ...
  const fetchPurchaseHistory = async () => {
    try {
      const response = await fetch(
        "https://cold-cassandra-backendsforpracticealltheverybestrukja-1e03db87.koyeb.app/purchasehistory",
        {
          method: "POST",
          headers: {
    ...
    <Fragment>
      <div className="flex flex-col gap-[5rem] items-center justify-center bg-gradient-to-b from-base-100 to-black pb-[5rem]">
        <h1 className="text-7xl mt-9 font-bold text-white">
          <span className="gradient-text animate-gradient">Purchase</span>{" "}
          History
        </h1>
        <div className="flex items-center flex-col gap-[2rem] justify-center w-[100%]">
