...
export default function OrganiseHistory({ user }) {
    ...
    const fetchOrganisingHistory = async () => {
      try {
        const response = await fetch(
          `https://cold-cassandra-backendsforpracticealltheverybestrukja-1e03db87.koyeb.app/organisehistory/${user}`
        );
    ...
    <Fragment>
      <div className="bg-black flex flex-col items-center justidy-center gap-[3rem]  bg-gradient-to-b from-base-100 to-black pb-[3rem]">
        <h1 className="text-7xl font-bold mt-7">
          <span className="gradient-text animate-gradient">Organising</span>{" "}
          History
        </h1>
